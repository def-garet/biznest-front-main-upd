...
const SRPMonitoring = () => {
  ...
  // Limit display to top 5 items for the Home Widget to keep it compact
  const PREVIEW_LIMIT = 5; 

  const fetchSrpData = async () => {
    try {
      const response = await axios.get(srpi_api);
      // Optional: Sort by price change magnitude or recency if available
      setSrpData(response.data); 
    } catch (err) {
      console.error("Error fetching SRP data:", err.message);
    } finally {
  ...
  const getPriceChangeColor = (change) => {
    if (change > 0) return "#ff4d4d"; // Red for price increase
    if (change < 0) return "#00b894"; // Green for price drop
    return "#b2bec3"; // Grey for no change
  };
  ...
  // Display only the first few items for the widget
  const widgetData = srpData.slice(0, PREVIEW_LIMIT);
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <View style={styles.tableHeader}>
        ...
        <Text style={[styles.headerText, styles.colPrice]}>SRP</Text>
        <Text style={[styles.headerText, styles.colTrend]}>TREND</Text>
      </View>
      ...
      {widgetData.length > 0 ? (
          ...
          return (
            <View key={item.id || index} style={styles.row}>
              ...
              <View style={styles.colProduct}>
                <Text style={styles.productName} numberOfLines={1}>{item.name}</Text>
                <Text style={styles.categoryName} numberOfLines={1}>{item.category || "General"}</Text>
              </View>
              ...
              <View style={styles.colTrend}>
                <View style={[styles.trendBadge, { backgroundColor: changeColor + '15' }]}>
                  <Ionicons name={getTrendIcon(priceChange)} size={14} color={changeColor} />
        ...
        <View style={styles.emptyState}>
          <Text style={styles.emptyText}>No price updates available.</Text>
        </View>
      )}
      ...
      <View style={styles.footer}>
        <Text style={styles.footerText}>
            Updated Today • <Text style={{color: COLORS.secondary}}>See full list</Text>
        </Text>
      </View>
...
const styles = StyleSheet.create({
  container: {
    // No outer padding/shadows here; allows Home card styling to take precedence
    width: '100%',
  },
  loadingContainer: {
    padding: 20,
  ...
  // Column Flex
  colProduct: { flex: 3, paddingRight: 5 },
  colPrice: { flex: 1.5, alignItems: 'flex-end' },
  colTrend: { flex: 1.5, alignItems: 'flex-end' },
